...
def main():
    id = json.loads(sys.argv[1])["id"]
    dataDir = json.loads(sys.argv[1])["storage-file"]["data-directory"]
    if id == "centos":
        rsyncSource = "rsync://mirrors.tuna.tsinghua.edu.cn/centos"
    elif id == "centos-altarch":
        rsyncSource = "rsync://mirrors.tuna.tsinghua.edu.cn/centos-altarch"
    elif id == "centos-vault":
        rsyncSource = "rsync://mirrors.tuna.tsinghua.edu.cn/centos-vault"
    else:
        assert False
    _Util.cmdExec("/usr/bin/rsync", "-v", "-a", "-z", "-H", "--delete", rsyncSource, dataDir)
